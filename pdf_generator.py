"""
VOXMILL EXECUTIVE PDF GENERATOR - READABLE BLACK EDITION
=========================================================
A4 portrait, paneled charts, #121212 chart backgrounds
Full ReportLab + matplotlib workflow preserved
"""
...
BG_CONTENT = colors.HexColor('#0F0F0F')
...
class VoxmillCanvas(canvas.Canvas):
    """Executive canvas with readable content area"""
    ...
    def save(self):
        ...
        for i, page in enumerate(self.pages):
            ...
            # Draw lighter content area (0.8" to 10.6" on A4)
            self.setFillColor(BG_CONTENT)
            self.rect(45, 0.8*inch, A4[0]-90, 9.8*inch, fill=1, stroke=0)
            
            # Restore page content and draw template
    ...
    def draw_template(self, page_num, page_count):
        """Draw header and footer on A4"""
        
        # Top gold line
        self.setStrokeColor(GOLD)
        self.setLineWidth(1)
        self.line(50, A4[1] - 60, A4[0] - 50, A4[1] - 60)
        
        # Logo
        if self.logo_path:
            try:
                self.drawImage(self.logo_path, A4[0]/2 - 25, A4[1] - 50, 
        ...
        self.drawCentredString(A4[0]/2, A4[1] - 90, "VOXMILL MARKET INTELLIGENCE")
        
        # Footer
        self.setStrokeColor(GOLD)
        self.setLineWidth(1)
        self.line(50, 0.6*inch, A4[0] - 50, 0.6*inch)
        
        self.setFont('Helvetica', 8)
        self.setFillColor(BRONZE)
        footer_text = f"VOXMILL AUTOMATIONS — CONFIDENTIAL | {datetime.now().year}"
        self.drawCentredString(A4[0]/2, 0.42*inch, footer_text)
        
        self.setFillColor(TEXT_MUTED)
        self.drawRightString(A4[0] - 50, 0.42*inch, f"PAGE {page_num} OF {page_count}")

def panel(content, padding=14, bg=BG_PANEL):
    """Wrap content in readable panel with gold border"""
    # A4 width minus margins = ~7.3 inches
    panel_width = A4[0]/inch - 1.4
    t = Table([[content]], colWidths=[panel_width*inch])
...
def create_chart_kpi(data):
    """KPI chart - #121212 background for visibility"""
    ...
    fig = plt.figure(figsize=(10, 3.5), facecolor='#121212')
    ...
    ax1.set_xticks([0])
    ax1.set_xticklabels(['ANALYZED'], color='#EAEAEA', fontsize=10, weight='600')
    ax1.set_ylabel('PROPERTIES', color='#B08D57', fontsize=9, weight='600')
    ax1.tick_params(colors='#EAEAEA', width=0)
    for spine in ax1.spines.values():
        spine.set_color('#2E2E2E')
    ax1.spines['top'].set_visible(False)
    ax1.spines['right'].set_visible(False)
    ax1.grid(axis='y', alpha=0.2, color='#2E2E2E', linestyle='-', linewidth=0.5)
    ...
    ax2.tick_params(colors='#EAEAEA', width=0)
    for spine in ax2.spines.values():
        spine.set_color('#2E2E2E')
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)
    ax2.grid(axis='x', alpha=0.2, color='#2E2E2E', linestyle='-', linewidth=0.5)
    
    plt.tight_layout()
    path = '/tmp/kpi.png'
    plt.savefig(path, dpi=200, bbox_inches='tight', facecolor='#121212', edgecolor='none')
    plt.close()
    return path

def create_chart_performance(data):
    """Performance charts - #121212 background"""
    
    properties = data['properties'][:10]
    
    plt.style.use('dark_background')
    fig = plt.figure(figsize=(10, 5.5), facecolor='#121212')
    ...
    ax1.spines['top'].set_visible(False)
    ax1.spines['right'].set_visible(False)
    ax1.grid(axis='x', alpha=0.2, color='#2E2E2E')
    ...
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)
    ax2.grid(alpha=0.2, color='#2E2E2E')
    ...
    ax4.set_ylim(0, 10)
    ax4.tick_params(colors='#EAEAEA', width=0)
    for spine in ax4.spines.values():
        spine.set_color('#2E2E2E')
    ax4.spines['top'].set_visible(False)
    ax4.spines['right'].set_visible(False)
    ax4.grid(axis='y', alpha=0.2, color='#2E2E2E')
...
def create_chart_competitors(data):
    """Competitor chart - #121212 background"""
    ...
    ax1.set_yticklabels(names, color='#EAEAEA', fontsize=8)
...
def create_pdf(data):
    """Create executive PDF on A4 with readable panels"""
    
    print("\n" + "="*70)
    print("VOXMILL EXECUTIVE PDF GENERATOR - A4 READABLE")
    print("="*70)
    
    # A4 portrait with slightly larger top margin
    doc = SimpleDocTemplate(OUTPUT_FILE, pagesize=A4, leftMargin=50, rightMargin=50, 
                           topMargin=1.1*inch, bottomMargin=0.8*inch)
    
    story = []
    styles = getSampleStyleSheet()
    
    # Premium typography - larger sizes, better line-height
    title_style = ParagraphStyle('Title', parent=styles['Heading1'], fontSize=28, 
                                 textColor=TEXT_LIGHT, spaceAfter=8, fontName='Times-Bold', 
                                 leading=32, alignment=TA_CENTER, spaceBefore=20)
    
    subtitle_style = ParagraphStyle('Subtitle', parent=styles['Normal'], fontSize=14, 
                                    textColor=BRONZE, spaceAfter=18, fontName='Helvetica-Bold', 
                                    leading=20, alignment=TA_CENTER, wordSpace=1.5)
    
    section_style = ParagraphStyle('Section', parent=styles['Heading2'], fontSize=18, 
                                   textColor=BRONZE, spaceBefore=20, spaceAfter=16, 
                                   fontName='Times-Bold', leading=22, alignment=TA_CENTER)
    
    body_style = ParagraphStyle('Body', parent=styles['Normal'], fontSize=12, 
                                textColor=TEXT_LIGHT, spaceAfter=16, leading=20, 
    ...
    story.append(panel(Image(kpi, width=6.8*inch, height=2.4*inch)))
    story.append(Spacer(1, 0.15*inch))
    
    # Metrics table with gold top border
    tbl_data = [
    ...
    panel_width = A4[0]/inch - 1.4
    tbl = Table(tbl_data, colWidths=[panel_width/4*inch]*4)
    ...
    story.append(panel(Image(perf, width=6.8*inch, height=3.7*inch)))
    story.append(Spacer(1, 0.1*inch))
    story.append(Paragraph("KEY INSIGHTS", section_style))
    summ = intel.get('executive_summary', 'Market analysis in progress.')
    # Wrap insights paragraph in panel for readability
    insights_para = Paragraph(summ, body_style)
    story.append(panel(insights_para, padding=16))
    story.append(PageBreak())
    ...
    story.append(panel(Image(comp, width=6.8*inch, height=2.4*inch)))
    story.append(PageBreak())
    ...
    intel_para = Paragraph(intel.get('executive_summary', 'Analysis in progress.'), body_style)
    story.append(panel(intel_para, padding=16))
    story.append(PageBreak())
    ...
    col_widths = [2.8*inch, 1.4*inch, 0.9*inch, 0.9*inch, 0.8*inch]
    opp_tbl = Table(opp_data, colWidths=col_widths)
