...
try:
    ...
    if MONGODB_URI:
        ...
        if needs_sync:
            AIRTABLE_API_KEY = os.getenv('AIRTABLE_API_KEY')
            AIRTABLE_BASE_ID = os.getenv('AIRTABLE_BASE_ID')
            airtable_table = client_profile.get('airtable_table', 'Accounts')  # ✅ FIXED
...
governance_result = await ConversationalGovernor.govern(
    ...
    system_state={
        'subscription_active': client_profile.get('subscription_status') == 'active',  # ✅ FIXED: lowercase
        'pin_unlocked': True,
        'quota_remaining': 100,
...
if governance_result.intent in [Intent.META_AUTHORITY, Intent.PROFILE_STATUS]:
    ...
    if governance_result.response:
        ...
        await sync_usage_metrics(
            ...
            table_name=client_profile.get('airtable_table', 'Accounts'),  # ✅ FIXED
...
if not query_region or len(query_region) < 3:
    query_region = client_profile.get('active_market', 'Mayfair')  # ✅ FIXED: Use active_market
