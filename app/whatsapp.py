...
Standing by."""
        ...
        # ====================================================================
        # PORTFOLIO ADD DETECTION (CHATGPT FIX - CRITICAL)
        # ====================================================================
        
        # ✅ NO KEYWORDS: Check if governance classified as PORTFOLIO_MANAGEMENT
        if governance_result.intent == Intent.PORTFOLIO_MANAGEMENT:
            logger.info(f"📊 Portfolio action detected by governance")
            
            # Check for implied property reference from conversation context
            conversation = ConversationSession(sender)
            last_messages = conversation.get_last_n_messages(3)
            
            implied_property = None
            message_lower = message_text.lower()
            
            # Check for reference words: "it", "this", "that"
            has_reference = any(ref in message_lower for ref in ['it', 'this', 'that'])
            
            if has_reference:
                # Look for property mentions in last 3 messages
                for msg in reversed(last_messages):
                    assistant_text = msg.get('assistant', '')
                    
                    # Extract property name from assistant's previous response
                    # Common patterns: "One Hyde Park", "property in [location]", etc.
                    
                    # Pattern 1: "One Hyde Park" (landmark)
                    if 'One Hyde Park' in assistant_text:
                        implied_property = {
                            'address': 'One Hyde Park, Knightsbridge',
                            'region': 'Knightsbridge',
                            'postcode': 'SW1X'
                        }
                        logger.info(f"✅ Implied property: One Hyde Park")
                        break
                    ...
                    if address_match:
                        ...
                        logger.info(f"✅ Implied property: {address_match.group(1)}")
                        break
            
            # If we found an implied property, use it
            if implied_property:
                try:
                    from app.portfolio import add_property_to_portfolio
                    
                    # Add to portfolio
                    response = add_property_to_portfolio(sender, implied_property)
                    ...
                    conversation.update_session(
                        user_message=message_text,
                        assistant_response=response,
                        metadata={'category': 'portfolio_management', 'intent': 'portfolio_add'}
                    )
                    ...
                    logger.info(f"✅ Portfolio property added: {implied_property['address']}")
                    return
                    
                except Exception as e:
                    logger.error(f"❌ Portfolio add error: {e}", exc_info=True)
                    response = "Failed to add property. Please try again."
                    await send_twilio_message(sender, response)
                    return
            
            # No implied property - need explicit details
            response = """PROPERTY DETAILS REQUIRED

Send property in one of these formats:
...
"""
            ...
            conversation.update_session(
                user_message=message_text,
                assistant_response=response,
                metadata={'category': 'portfolio_prompt'}
            )
            
            logger.info(f"⚠️ Portfolio add failed: no property details found")
            return
